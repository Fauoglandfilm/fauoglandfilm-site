...
const OSLO_PARKOUR_PARK_PASSWORD = "Oslo_Parkour_Park_2026";
const OSLO_PARKOUR_PARK_SESSION_STORAGE_KEY =
  "fauoglandfilm-oslo-parkour-park-pitch-unlocked";
const OSLO_PARKOUR_PARK_CANVA_EMBED_SRC =
  "https://www.canva.com/design/DAHIsEgC9ME/vWZkWp0c8AcM4SEI8E2H5Q/view?embed";
...
function getInitialUnlockedState() {
  ...
  try {
    return window.sessionStorage.getItem(OSLO_PARKOUR_PARK_SESSION_STORAGE_KEY) === "true";
  } catch {
    return false;
  }
}

export function OsloParkourParkPitchPageContent() {
  ...
  const handleSubmit = (event: React.FormEvent<HTMLFormElement>) => {
    ...
    if (password === OSLO_PARKOUR_PARK_PASSWORD) {
      setIsUnlocked(true);
      setErrorMessage("");
      setPassword("");

      try {
        window.sessionStorage.setItem(OSLO_PARKOUR_PARK_SESSION_STORAGE_KEY, "true");
      } catch {}
  ...
  const handleLock = () => {
    ...
    try {
      window.sessionStorage.removeItem(OSLO_PARKOUR_PARK_SESSION_STORAGE_KEY);
    } catch {}
  };
            ...
            <div className="relative flex items-center justify-between gap-4 border-b border-white/10 px-4 py-3.5 sm:px-5 sm:py-4">
              <div className="min-w-0">
                ...
                <p className="mt-2 text-[0.68rem] font-semibold uppercase tracking-[0.22em] text-white/58">
                  Oslo Parkour Park client presentation
                </p>
              </div>
            ...
            <div className="relative min-h-0 flex-1 p-2.5 sm:p-3.5">
              <div className="relative h-full w-full overflow-hidden rounded-[1.35rem] border border-white/10 bg-[#0a121d] shadow-[inset_0_1px_0_rgba(255,255,255,0.06)] sm:rounded-[1.6rem]">
                {externalMediaEnabled ? (
                  <iframe
                    src={OSLO_PARKOUR_PARK_CANVA_EMBED_SRC}
                    title="Oslo Parkour Park client presentation"
                    loading="eager"
                    allow="fullscreen"
                    allowFullScreen
                  ...
                  <ExternalMediaConsentPlaceholder sourceUrl={OSLO_PARKOUR_PARK_CANVA_EMBED_SRC} />
                )}
            ...
            <div className="relative px-5 py-6 sm:px-7 sm:py-8">
              ...
              <div className="mt-7 space-y-3 text-center sm:mt-8">
                ...
                <h1 className="font-display text-[clamp(2rem,7vw,3rem)] leading-[0.96] tracking-[-0.055em] text-white">
                  Open the Oslo Parkour Park presentation.
                </h1>
                <p className="mx-auto max-w-[24rem] text-[0.95rem] leading-7 text-white/68 sm:text-base">
                  Enter the client passcode to open the full-screen presentation experience.
                </p>
              </div>
