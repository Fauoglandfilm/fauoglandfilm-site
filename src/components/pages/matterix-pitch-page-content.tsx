...
const MATTERIX_PASSWORD = "Matterix2026";
const MATTERIX_SESSION_STORAGE_KEY = "fauoglandfilm-matterix-pitch-unlocked";
const MATTERIX_CANVA_EMBED_SRC =
  "https://www.canva.com/design/DAHHHczeIIU/vtlhaD71gHdQ5HHg5m3qpQ/view?embed";
...
function getInitialUnlockedState() {
  ...
  try {
    return window.sessionStorage.getItem(MATTERIX_SESSION_STORAGE_KEY) === "true";
  } catch {
    return false;
  }
}

export function MatterixPitchPageContent() {
  ...
  const handleSubmit = (event: React.FormEvent<HTMLFormElement>) => {
    ...
    if (password === MATTERIX_PASSWORD) {
      setIsUnlocked(true);
      setErrorMessage("");
      setPassword("");

      try {
        window.sessionStorage.setItem(MATTERIX_SESSION_STORAGE_KEY, "true");
      } catch {}
  ...
  const handleLock = () => {
    ...
    try {
      window.sessionStorage.removeItem(MATTERIX_SESSION_STORAGE_KEY);
    } catch {}
  };
            ...
            <div className="relative min-h-0 flex-1 p-2.5 sm:p-3.5">
              <div className="relative h-full w-full overflow-hidden rounded-[1.35rem] border border-white/10 bg-[#0a121d] shadow-[inset_0_1px_0_rgba(255,255,255,0.06)] sm:rounded-[1.6rem]">
                {externalMediaEnabled ? (
                  <iframe
                    src={MATTERIX_CANVA_EMBED_SRC}
                    title="Matterix client presentation"
                    loading="eager"
                    allow="fullscreen"
                    allowFullScreen
                  ...
                  <ExternalMediaConsentPlaceholder sourceUrl={MATTERIX_CANVA_EMBED_SRC} />
                )}
            ...
            <div className="relative px-5 py-6 sm:px-7 sm:py-8">
              ...
              <div className="mt-7 space-y-3 text-center sm:mt-8">
                ...
                <h1 className="font-display text-[clamp(2rem,7vw,3rem)] leading-[0.96] tracking-[-0.055em] text-white">
                  Open the Matterix presentation.
                </h1>
